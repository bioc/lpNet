.doILP_steadyStateV2 <- function(obs, delta, lambda, b, n, K, T_=NULL, annot, delta_type, prior=NULL, 
							...
							if (all.pos) { 
								...
								for (j in J[J!=i]) {  # sum
									idPos <- which(annot == paste("w+", j, i, sep="_"))  # positive parameter
										...
										if (!is.na(obs[j,k])) {
											if (obs[j,k] >= delta[j] & b[(k-1)*n + j] == 1) {
												W[count,idPos] <- obs[j,k]
											}
											else {
												W[count,idPos] <- 0
											}
					...
					if (obs[i,k]>= delta[i,k]) {  # if observation of gene i after knockdown k is active
						if (all.pos) {
							...
							for (j in J[J!=i]) {  # sum
								...
								if (!is.na(obs[j,k])) {
									if (obs[j,k] >= delta[j,k] & b[(k-1)*n + j] == 1) {
										W[count,idPos] <- obs[j,k]
									}
									else {
										W[count,idPos] <- 0
