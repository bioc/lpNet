...
\title{lpNet}
\author{Bettina Knapp, Johanna Mazur and Lars Kaderali}
...
This package infers networks using perturbation data with a linear optimization program as published in \cite{knapp12}. In section \ref{data}, the idea of the network inference method and the type of data which can be used is explained. Section \ref{start} shows how to use the package on a quick example. 
In section \ref{loocv} and \ref{kfoldcv}, Leave-One-Out cross-validation (LOOCV) and k-fold cross-validation is used to find the best parameter $\lambda$ which minimizes the mean squared error (MSE) of the used objective function as described in \cite{knapp12}. Section \ref{prior} exemplifies how prior knowledge can be included in the network inference process.
...
The method proposed in this package aims at reconstructing intracellular signal transduction networks from perturbation data. 
We consider signal transduction as an information flow through a network where nodes represent genes (or proteins) and edges represent the continuous strengths of interactions between the nodes (genes/proteins). 
...
Then, define details about the perturbation data which is used for the network inference. For this purpose, define the number of genes and the number of perturbation experiments which are given. Thereafter, we define a random observation matrix and define threshold $\delta_i$ as the mean of the observations of each gene $i\in \{1,\dots,n\}$.
<<data1, results=hide>>=
n <- 5 # number of genes
...
<<lp, results=hide>>=
res1 <- doILP(obs,delta,lambda=1,b,n,K,annot=getEdgeAnnot(n))
...
<<lp_pos, results=hide>>=
res2 <- doILP(obs,delta,lambda=1,b,n,K,
...
First, the parameters (mean and standard deviation) for the two Gaussian distributions representing activation and deactivation have to be defined.
...
<<loocv, results=hide>>=
adja_loocv <- getSampleAdjaMAD(loocv_res$edges_all,n,annot_node)
@
...
lambda <- calcRangeLambda(delta,obs,stepsize=2)
MSE <- Inf
for(lamd in lambda){
...
<<prior1, results=hide>>=
res3 <- doILP(obs,delta,lambda=1,b,n,K,
              annot=getEdgeAnnot(n),sourceNode=1,sinkNode=5)
adja3 <- getAdja(res3,n)
@
Similary, additional constraints defining prior knowledge of individual edges can be formulated and added as a list to the ``doILP" function. 
...
resERBB <- doILP(dataSt[,-1],delta,lambda=1.83,b,n,K,annot,
